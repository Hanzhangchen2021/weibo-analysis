...
class SearchSpider(scrapy.Spider):
    name = 'search'
    allowed_domains = ['weibo-search.com']
        ...
        for keyword in self.keyword_list:
            if not self.settings.get('REGION') or '全部' in self.settings.get(
                ...
                for region in self.regions.values():
                    ...
                    url += self.contain_type
                    url += '&timescope=custom:{}:{}'.format(start_str, end_str)
                    # Get search results for a province
                    yield scrapy.Request(url=url,
                                         callback=self.parse,
                                         meta={
                                             'base_url': base_url,
    ...
    def check_environment(self):
        """Determine whether the software required by the configuration has been installed"""
    ...
    def parse_by_day(self, response):
        ...
        if is_empty:
            ...
            for i in range(1, 25):
                ...
                url += '&timescope=custom:{}:{}&page=1'.format(
                    start_str, end_str)
                # Get one hour of search results
                yield scrapy.Request(url=url,
    ...
    def parse_by_hour(self, response):
        ...
        if is_empty:
            ...
            for region in self.regions.values():
                ...
                url += '&timescope=custom:{}:{}&page=1'.format(
                    start_time, end_time)
                # Get search results for one province per hour
                yield scrapy.Request(url=url,
                                     callback=self.parse_by_hour_province,
                                     meta={
                                         'keyword': keyword,
    ...
    def parse_by_hour_province(self, response):
        """Filter by hour and municipality/province"""
        keyword = response.meta.get('keyword')
        is_empty = response.xpath(
            '//div[@class="card card-no-result s-pt20b40"]')
        start_time = response.meta.get('start_time')
        ...
        if is_empty:
            ...
            for city in province['city'].values():
                ...
                url += '&timescope=custom:{}:{}&page=1'.format(
                    start_time, end_time)
                # Get search results for one city per hour
                yield scrapy.Request(url=url,
                                     callback=self.parse_page,
                                     meta={
                                         'keyword': keyword,
    ...
    def parse_page(self, response):
        """Parse the information of a page of search results"""
        keyword = response.meta.get('keyword')
        is_empty = response.xpath(
            '//div[@class="card card-no-result s-pt20b40"]')
        if is_empty:
    ...
    def get_article_url(self, selector):
        """Get the url of Weibo headline article"""
        article_url = ''
    ...
    def get_at_users(self, selector):
        """Get the user nickname of @ in Weibo"""
        a_list = selector.xpath('.//a')
        at_users = ''
        at_list = []
        for a in a_list:
    ...
    def get_topics(self, selector):
        """Get participating Weibo topics"""
        a_list = selector.xpath('.//a')
        topics = ''
        topic_list = []
        for a in a_list:
    ...
    def parse_weibo(self, response):
        """Parse the Weibo information in the webpage"""
        keyword = response.meta.get('keyword')
        for sel in response.xpath("//div[@class='card-wrap']"):
            ...
            if info:
                ...
                if retweet_sel and retweet_sel[0].xpath(
                    ...
                    retweet['retweet_id'] = ''
                    yield {'weibo-search': retweet, 'keyword': keyword}
                    weibo['retweet_id'] = retweet['id']
                print(weibo)
                yield {'weibo-search': weibo, 'keyword': keyword}
