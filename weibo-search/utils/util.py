...
def convert_contain_type(contain_type):
    """Convert the containing type to a string"""
    if contain_type == 0:
        return '&suball=1'
    elif contain_type == 1:
        return '&haspic=1'
...
def get_regions(region):
    """Return the region that meets the requirements according to the regional filter criteria"""
    new_region = {}
    if region:
        for key in region:
...
def standardize_date(created_at):
    """Standardized Weibo release time"""
    if "刚刚" in created_at:
